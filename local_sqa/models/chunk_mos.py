...
class ChunkMOS(SSLMOS):
    ...
    def encode(
        ...
        *,
        ...
        segments, segment_lengths, segs_per_wav, _ = self.segment(
            latents, self.segment_length, seq_len_latents,
            axis=1,
        )
        xs = self.encoder.extract_features_from_latents(
            segments, segment_lengths.numpy()
        )
        target_sequence_lengths = self.encoder.compute_output_lengths(
            num_samples
        )
        if target_sequence_lengths is None:
            if seq_len_latents is None:
                seq_len_latents = [latents.shape[1]]*len(latents)
            target_sequence_lengths = seq_len_latents
    ...
    def segment(
        ...
        segments = segments.moveaxis(-1, axis)
        return (
    ...
    def stitch(
        ...
        segments = segments.moveaxis(axis, 1)
